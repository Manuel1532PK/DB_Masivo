...
const SongTable = ({ songs, filters }) => {
  ...
  const processedSongs = [...songs]
    ...
    .sort((a, b) => {
      if (filters.order === 'alphabetical') return a.song_name.localeCompare(b.song_name);
        ...
        <tbody>
          {processedSongs.map((song, i) => (
            <tr key={i}>
              ...
              <td>{song.artist}</td>
              <td><span className="badge">{song.genre || 'N/A'}</span></td>
              <td>
                <div className="pop-container">
                  <div className="pop-bar" style={{ width: `${song.popularity}%` }}></div>
                  <span>{song.popularity}%</span>
                </div>
              </td>
              <td>{Math.floor(song.duration_sec / 60)}:{(song.duration_sec % 60).toString().padStart(2, '0')}</td>
            </tr>
          ))}
        </tbody>
      </table>
